...
export async function getEvents(options: GetEventsOptions = {}) {
  ...
  const { eq, ilike, and, or, asc, desc, gte, lte } = await import('drizzle-orm');
  const whereClauses = [];
